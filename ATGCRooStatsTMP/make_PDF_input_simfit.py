...
parser.add_option('--simfit', action='store_true', dest='simfit', default=False, help='save pdfs and norms for simultaneous fit')
...
def make_plots(rrv_x,wtmp,ch,cat,channel,fitres,binlo,binhi,binlo4fit):
	...
	for i in range(3):
		...
		wtmp.data('SMdatahist4fit').plotOn(plots[i],RooFit.MarkerColor(kBlack),RooFit.LineColor(kBlack),RooFit.DataError(RooAbsData.SumW2),RooFit.DrawOption('E0'),RooFit.Name('SMdata'))
		normvalSM	= norm.getVal() * wtmp.data('SMdatahist4fit').sumEntries()

		wtmp.pdf('aTGC_model_%s'%channel).plotOn(plots[i],RooFit.LineColor(kBlack),RooFit.Normalization(normvalSM, RooAbsReal.NumEvent),RooFit.Name('SMmodel'))

		wtmp.data('neg_datahist_%s'%POI[i]).plotOn(plots[i],RooFit.MarkerColor(kBlue),RooFit.LineColor(kBlue),RooFit.DataError(RooAbsData.SumW2),RooFit.DrawOption('E0'),RooFit.Name('atgcdata'))
		wtmp.var(POI[i]).setVal(-par_max[POI[i]])
		normvalneg = norm.getVal() * wtmp.data('SMdatahist4fit').sumEntries()
		wtmp.pdf('aTGC_model_%s'%channel).plotOn(plots[i],RooFit.LineColor(kBlue),RooFit.Normalization(normvalneg, RooAbsReal.NumEvent),RooFit.Name('atgcmodel'))
		...
		plots[i].Draw()
		ndof	= (binhi-binlo)/100 - 4
		plots[i].Print()
		print calculate_chi2(wtmp.data('neg_datahist_%s'%POI[i]),rrv_x,pullhist,plots[i],ndof,1)
		print plots[i].GetNbinsX()
		plots[i].Print()
		raw_input(POI[i])
		...
		leg.AddEntry(plots[i].findObject('atgcdata'),'MC '+parlatex[i]+'='+str(-par_max[POI[i]])+' TeV^{-2}','le')
		leg.AddEntry(plots[i].findObject('atgcmodel'),'signal model '+parlatex[i]+'='+str(-par_max[POI[i]])+' TeV^{-2}','l')
		leg.Draw()
		leg.Print()
...
#i
	...
	fileInWs	= TFile.Open('Input/wwlvj_%s_HP%s_workspace.root'%(ch[:2],cat[1]))
	w		= fileInWs.Get('workspace4limit_') 
	w.pdf('STop_xww_%s_HP%s'%(ch,cat[1])).SetName('mlvj_STop_%s'%channel)
	w.pdf('TTbar_xww_%s_HP%s'%(ch,cat[1])).SetName('mlvj_TTbar_%s'%channel)
	w.pdf('WJets_xww_%s_HP%s'%(ch,cat[1])).SetName('mlvj_WJets_%s'%channel)
	w.pdf('VV_xww_%s_HP%s'%(ch,cat[1])).SetName('mlvj_VV_%s'%channel)
	w.pdf('model_pdf_WJets0_xww_sb_lo_from_fitting_%s_mlvj'%ch).SetName('model_pdf_WJets_xww_sb_%s_mlvj'%ch)
	w.pdf('model_pdf_TTbar_xww_sb_lo_%s_mlvj'%ch).SetName('model_pdf_TTbar_xww_sb_%s_mlvj'%ch)
	w.pdf('model_pdf_STop_xww_sb_lo_%s_mlvj'%ch).SetName('model_pdf_STop_xww_sb_%s_mlvj'%ch)
	w.pdf('model_pdf_VV_xww_sb_lo_%s_mlvj'%ch).SetName('model_pdf_VV_xww_sb_%s_mlvj'%ch)
	w.var("rrv_number_WJets0_xww_%s_mj_postfit"%ch).SetName("rrv_number_WJets_xww_%s_mj_postfit"%ch)
	w.var("rrv_c_Exp_STop_xww_signal_region_%s"%ch).SetName("rrv_c_Exp_STop_xww_signal_region_%s"%channel)



	#these are postfit values (except VV)
	...
	if options.simfit:
	  ...
	  getattr(WS,'import')(w.data('dataset_mj_sb_lo'))
	  getattr(WS,'import')(w.data('dataset_mj_sb_hi'))
	  getattr(WS,'import')(w.data('dataset_mj_%s_sig'%cat))

	  #make bkg pdfs and norm vars

	  w.var('rrv_mass_j').setRange(cat,mj_lo,mj_hi)
	  w.var('rrv_mass_j').setRange('sb_lo',40,65)
	  w.var('rrv_mass_j').setRange('sb_hi',105,150)


	  bkgs		= ['WJets','TTbar','VV','STop']
	  regions	= [cat+'_sig','sb_lo','sb_hi']

	  for region in regions:
	      WS2	= WS.Clone("w")
	      for bkg in bkgs:
		  if bkg=='VV':
		    norm_var	= RooRealVar('normvar_%s_%s'%(bkg,ch),'normvar_%s_%s'%(bkg,ch),w.var("rrv_number_%s_xww_%s_mj_prefit"%(bkg,ch)).getVal())
		  else:
		    norm_var	= RooRealVar('normvar_%s_%s'%(bkg,ch),'normvar_%s_%s'%(bkg,ch),w.var("rrv_number_%s_xww_%s_mj_postfit"%(bkg,ch)).getVal())
		  reg_Int		= w.pdf('m_j_%s_%s'%(bkg,ch)).createIntegral(RooArgSet(w.var('rrv_mass_j')),RooArgSet(w.var('rrv_mass_j')), region)
		  if bkg=='WJets':
		    norm_var.setConstant(kFALSE)
		    norm		= RooFormulaVar('%s_%s_%s_norm'%(bkg,region,ch),'%s_%s_%s_norm'%(bkg,region,ch),'@0*@1',RooArgList(reg_Int,norm_var))
		  else:
		    norm		= RooFormulaVar('%s_%s_%s_norm'%(bkg,region,ch),'%s_%s_%s_norm'%(bkg,region,ch),'%s*@0'%reg_Int.getVal(),RooArgList(norm_var))
		    norm_var.setConstant(kTRUE)	
		  if cat in region:
		    bkg_MWV	= w.pdf('mlvj_%s_%s'%(bkg,channel)).clone('mlvj_%s_%s_%s'%(bkg,region,ch))
		  else:
		    if bkg=='WJets':
		      bkg_MWV	= w.pdf('model_pdf_WJets_xww_sb_%s_mlvj'%ch).clone('mlvj_WJets_sb_%s'%ch)
		      params_mlvj	= bkg_MWV.getParameters(WS.data('dataset_mj_%s'%region))
		      p_iter	= params_mlvj.createIterator()
		      p_iter.Reset()
		      param	= p_iter.Next()
		      while param:
			param.setConstant(kFALSE)
			#param.setRange(-5000,10000)
			param	= p_iter.Next()
		    else:
		      bkg_MWV	= w.pdf('model_pdf_%s_xww_sb_%s_mlvj'%(bkg,ch)).clone('mlvj_%s_sb_%s'%(bkg,ch))
		  bkg_mj	= w.pdf('m_j_%s_%s_pdf_%s'%(bkg,region,ch)).clone('mj_%s_%s_%s'%(bkg,region,ch))
		  ###add el/mu to mj parameter names to have different parameters in the simultaneous fit
		  params_mj	= bkg_mj.getParameters(WS.data('dataset_mj_%s'%region))
		  p_iter	= params_mj.createIterator()
		  p_iter.Reset()
		  param 	= p_iter.Next()
		  while param:
		    if ch not in param.GetName():
		      param.SetName(param.GetName()+'_'+ch)
		    #if param.GetName()!='rrv_c_ErfExp_WJets0_xww_%s'%ch:
		  ...
		  getattr(WS2,'import')(bkg_2d_pdf,RooFit.RecycleConflictNodes())
		  getattr(WS2,'import')(norm,RooFit.RecycleConflictNodes())
	      #signal function
	      sig_2d	= RooProdPdf('ATGCPdf_%s_%s'%(region,ch),'ATGCPdf_%s_%s'%(region,ch),RooArgList(WS.pdf('aTGC_model_%s'%channel),w.pdf('m_j_VV_%s_pdf_%s'%(region,ch))))
	      getattr(WS2,'import')(w.data('dataset_mj_%s'%region).Clone('dataset_mj_%s_%s'%(region,ch)))

	      norm_sig	= RooFormulaVar(sig_2d.GetName()+'_norm',sig_2d.GetName()+'_norm','@0*@1',RooArgList(WS2.function('VV_norm'),WS.function('normfactor_3d_%s'%channel)))

	      getattr(WS2,'import')(sig_2d,RooFit.RecycleConflictNodes())
	      getattr(WS2,'import')(norm_sig,RooFit.RecycleConflictNodes())

		    
	      #save sb only once
	      if cat in region:
		output	= TFile('%s/%s_%s.root'%(path,region,ch),'recreate')
		WS2.Write();
		model.Write()
		output.Close()
	...
	#save wtmp for plots
	fileOut4plots	= TFile.Open('wtmp4plots_%s'%channel,'recreate')
	wtmp.Write()
	fileOut4plots.Close()
	...
	if options.printatgc:
		...
		for i in range(3):
			for j in range(3):
				wtmp.var(POI[j]).setVal(0)
			wtmp.var(POI[i]).setVal(par_max[POI[i]])
			print channel + ' ' + POI[i] + ' : ' + str(w.var('rate_VV').getVal()*normfactor_3d.getVal())
