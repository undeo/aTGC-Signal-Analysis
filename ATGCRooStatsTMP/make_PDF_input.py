...
def make_input(ch = 'ele'):
	...
	#prepare variables and parameters and temporary workspace
	...
	#make and fit model
	if len(POI) == 1:
		paralist	= RooArgList(N1_ratio,wtmp.function(POI[0]+str(par_max[POI[0]])))
		N1_1d		= RooFormulaVar('N1_1d','N1_1d','1/(1+@0*@1)',paralist)
		N2_1d		= RooFormulaVar('N2_1d','N2_1d','(@0*@1)/(1+@0*@1)',paralist)		
		model 		= RooAddPdf('aTGC_model','aTGC_model',\
					    RooArgList(SMPdf,wtmp.pdf('Pdf_%s'%POI[0])),\
					    RooArgList(N1_1d,N2_1d))
	...
	if len(POI) == 2:
		paralist	= RooArgList(wtmp.function('N1_ratio_%s'%POI[0]),wtmp.function(POI[0]+str(par_max[POI[0]])),\
						wtmp.function('N1_ratio_%s'%POI[1]),wtmp.function(POI[1]+str(par_max[POI[1]])))
		...
		model		= RooAddPdf('aTGC_model','aTGC_model',\
					    RooArgList(SMPdf,wtmp.pdf('Pdf_%s'%POI[0]),wtmp.pdf('Pdf_%s'%POI[1])),\
					    RooArgList(N1_2d,N2_2d,N3_2d))
		model.Print()
		#fit first pdf
		wtmp.var(POI[0]).setVal(par_max[POI[0]])
		...
		fitres1.Print()
		fitres2.Print()
		normfactor_2d = RooFormulaVar('normfactor_2d_%s'%ch,'normfactor_2d_%s'%ch,'1+((@0-1)+(@1-1))',\
					      RooArgList(wtmp.function('scaleshape_%s_%s'%(POI[0],ch)),\
	...
	if len(POI) == 3:
		paralist	= RooArgList(wtmp.function('N1_ratio_%s'%POI[0]),wtmp.function(POI[0]+str(par_max[POI[0]])),\
					     wtmp.function('N1_ratio_%s'%POI[1]),wtmp.function(POI[1]+str(par_max[POI[1]])),\
					     wtmp.function('N1_ratio_%s'%POI[2]),wtmp.function(POI[2]+str(par_max[POI[2]])))
		...
		model		= RooAddPdf('aTGC_model','aTGC_model',\
					    RooArgList(SMPdf,wtmp.pdf('Pdf_%s'%POI[0]),wtmp.pdf('Pdf_%s'%POI[1]),wtmp.pdf('Pdf_%s'%POI[2])),\
					    RooArgList(N1_3d,N2_3d,N3_3d,N4_3d))
		normfactor_3d	= RooFormulaVar('normfactor_3d_%s'%ch,'normfactor_3d_%s'%ch,'1+(@0-1)+(@1-1)+(@2-1)',\
						RooArgList(wtmp.function('scaleshape_%s_%s'%(POI[0],ch)),\
							   wtmp.function('scaleshape_%s_%s'%(POI[1],ch)),\
		...
		if options.do_plots:
			...
			for i in range(3):
					wtmp.var(POI[i]).setVal(par_max[POI[i]])
			model.plotOn(p4,RooFit.LineColor(kRed),RooFit.Normalization(wtmp.function('normfactor_3d_%s'%ch).getVal()*wtmp.data('SMdatahist').sumEntries(),RooAbsReal.NumEvent))
			for j in range(14):
				for i in range(3):
