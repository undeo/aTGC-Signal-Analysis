...
{
  if (parname.EqualTo("cwww") )  return "c_{WWW}/#Lambda^{2} (1/TeV^{2})";
  if (parname.EqualTo("ccw") ) return "c_{W}/#Lambda^{2} (1/TeV^{2})";
  if (parname.EqualTo("cb") ) return "c_{B}/#Lambda^{2} (1/TeV^{2})";
...
float parmin(const TString& parname)
{
  ...
  if (parname.EqualTo("ccw") ) return -30;
  if (parname.EqualTo("cb") ) return -65;
		    ...
		    TLegend *legend)
{
  ...
  //text->AddText(Form("95%% CL Limit on %s and %s",par2latex(par1).Data(),par2latex(par2).Data()));
  text->AddText(0,0.35,Form("2.09 fb^{-1} (%d TeV)",beamcometev));
  text->Draw();
...
void atgcplotLimit()
{
  ...
  fnames.push_back("higgsCombine2Par_ccw0_cb0.MultiDimFit.mH120.root");
  ...
  */
  ...
  TLegend *legend = new TLegend(0.2,0.16,0.43,0.33,"","NDC");
