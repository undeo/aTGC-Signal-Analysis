...
def make_ATGCtree(fileInATGC='', ch='ele', category='WW', sigreg = 'lo'):

	fileInATGC	= TFile.Open(fileInATGC)
	treeInATGC	= fileInATGC.Get('treeDumper/BasicTree')
	fileOutATGC	= TFile('Output/ATGC-Tree_%s_%s_%s.root'%(sigreg,ch,category), 'recreate')
...
def make_input(ch = 'ele'):
	...
	for categ in ['WW','WZ']:
		if options.newtrees:
			make_ATGCtree('Input/%s-aTGC-%s.root'%(categ,ch), ch, categ, sigreg)
		fileInATGC	= TFile.Open('Output/ATGC-Tree_%s_%s_%s.root'%(sigreg,ch,categ))
		...
		for i in range(treeATGC.GetEntries()):
			...
			if treeATGC.c_wwwl == 0 and treeATGC.c_wl ==0 and treeATGC.c_bl == 0:
				...
				if categ == 'WW':
					SMhistWW.Fill(treeATGC.MWW,treeATGC.weight)
				if categ == 'WZ':
					SMhistWZ.Fill(treeATGC.MWW,treeATGC.weight)
		...
		for categ in ['WW','WZ']:
			...
			for i in range(treeATGC.GetEntries()):
				...
				or (para == 'cb' and treeATGC.c_wwwl == 0 and treeATGC.c_wl == 0 and treeATGC.c_bl == 60):
			      		pos_hist_all.Fill(treeATGC.MWW,treeATGC.weight)
			    	if (para == 'cwww' and treeATGC.c_wwwl == -12 and treeATGC.c_wl == 0 and treeATGC.c_bl == 0)\
				or (para == 'ccw' and treeATGC.c_wwwl == 0 and treeATGC.c_wl == -20 and treeATGC.c_bl == 0)\
				or (para == 'cb' and treeATGC.c_wwwl == 0 and treeATGC.c_wl == 0 and treeATGC.c_bl == -60):
			      		neg_hist_all.Fill(treeATGC.MWW,treeATGC.weight)
